...
This approach has been codified in the link:https://12factor.net/config[12-Factor App].

However, in most Clojure projects, configuration is done primarily in
link:https://github.com/edn-format/edn[EDN format].

This library provides a number of reader macros that allow a EDN configuration file
...
{:connection-pool
  {:user-name #dyn/prop [DB_USER "accountsuser"]
   ...
   :url  #dyn/join ["jdbc:postgresql://"
                       ...
                       "/accounts"]}
 :web-server
  {:port #dyn/long #dyn/port "WEB_PORT"}}
----
